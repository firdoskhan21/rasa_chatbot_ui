...
const TaskDescription = ({ task, user, handleTaskChange, completedTasks }) => {
  return (
    <Box>
      <div key={"desc"} className="task-stepper-item ">
        <h1
          className={`typeform-title ${completedTasks.includes(task) ? "completed-task" : ""
            }`}
        >
        ...
        <>
          {user?.tasks
            ...
            .map((task, index) => (
              ...
              <div
                className={`remaining-task-title  ${completedTasks.includes(task) ? "completed-task" : ""
                  }`}
                onClick={() => {
